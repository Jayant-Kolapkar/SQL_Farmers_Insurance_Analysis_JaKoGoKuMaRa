-- Group Name: JaKoGoKuMaRa
-- Assignment: SQL Farmers Insurance Analysis
-- Total Marks: 100

USE ndap;

-- ----------------------------------------------------------------------------------------------
-- SECTION 1. SELECT Queries [5 Marks]
-- ----------------------------------------------------------------------------------------------
...
-- --------------------------------------------------------------------------------------
-- SECTION 2. Filtering Data (WHERE) [15 Marks]
-- --------------------------------------------------------------------------------------
...
-- ------------------------------------------------------------------------------------------------
-- SECTION 3. Aggregation (GROUP BY) [10 marks]
-- ------------------------------------------------------------------------------------------------
...
-- -------------------------------------------------------------------------------------------------
-- SECTION 4. Sorting Data (ORDER BY) [10 Marks]
-- -------------------------------------------------------------------------------------------------
...
(SELECT srcStateName, srcDistrictName, SumInsured, FarmersPremiumAmount
    FROM FarmersInsuranceData
    WHERE FarmersPremiumAmount > 0 AND SumInsured > 0
...
SELECT 
    ...
    (SUM(TotalFarmersCovered) / SUM(TotalPopulation) * 100) AS FarmerToPopulationRatio
FROM FarmersInsuranceData
WHERE TotalPopulation > 0
GROUP BY srcStateName, srcYear
ORDER BY FarmerToPopulationRatio DESC
LIMIT 3;

-- ###

-- -------------------------------------------------------------------------------------------------
-- SECTION 5. String Functions [6 Marks]
-- -------------------------------------------------------------------------------------------------
...
SELECT
    ...
    f1.srcDistrictName,
    f1.srcYear,
    SUM(f1.FarmersPremiumAmount) AS AggregatedFarmersPremium
FROM FarmersInsuranceData f1
INNER JOIN (
    SELECT DISTINCT srcStateName, srcDistrictName
    FROM FarmersInsuranceData
    WHERE InsuranceUnits > 10
) f2 ON f1.srcStateName = f2.srcStateName
    AND f1.srcDistrictName = f2.srcDistrictName
GROUP BY f1.srcStateName, f1.srcDistrictName, f1.srcYear
ORDER BY SUM(f1.FarmersPremiumAmount) DESC;
...
INNER JOIN (
    ...
    FROM FarmersInsuranceData
    GROUP BY srcStateName, srcDistrictName
    HAVING MAX(FarmersPremiumAmount) > 2000  -- 2000 lakhs = 20 crores
) f2 ON f1.srcStateName = f2.srcStateName 
    AND f1.srcDistrictName = f2.srcDistrictName
ORDER BY f2.MaxFarmersPremium DESC;
...
SELECT 
    ...
    pop.srcDistrictName,
    pop.AvgPopulation,
    farmers.TotalFarmersPremium,
    farmers.TotalFarmersCovered,
    farmers.TotalSumInsured
FROM (
    SELECT 
        srcStateName,
        srcDistrictName,
...
LEFT JOIN (
    ...
    FROM FarmersInsuranceData
    GROUP BY srcStateName, srcDistrictName
    HAVING SUM(FarmersPremiumAmount) > 10000  -- 10000 lakhs = 100 crores
) farmers ON pop.srcStateName = farmers.srcStateName 
    AND pop.srcDistrictName = farmers.srcDistrictName
WHERE farmers.TotalFarmersPremium IS NOT NULL
ORDER BY farmers.TotalFarmersPremium DESC;

-- ###

-- -------------------------------------------------------------------------------------------------
-- SECTION 7. Subqueries [10 Marks]
-- -------------------------------------------------------------------------------------------------
...
SELECT srcStateName, SUM(SumInsured) AS TotalSumInsured FROM FarmersInsuranceData
GROUP BY srcStateName
HAVING SUM(SumInsured) > (SELECT SumInsured FROM FarmersInsuranceData
...
-- -------------------------------------------------------------------------------------------------
-- SECTION 8. Advanced SQL Functions (Window Functions) [10 Marks]
-- -------------------------------------------------------------------------------------------------
...
-- -------------------------------------------------------------------------------------------------
-- SECTION 9. Data Integrity (Constraints, Foreign Keys) [4 Marks]
-- -------------------------------------------------------------------------------------------------
...
SELECT DISTINCT StateCode, srcStateName AS StateName
FROM FarmersInsuranceData
...
SELECT DistrictCode, 
       MIN(srcDistrictName) AS DistrictName, 
...
ADD PRIMARY KEY (DistrictCode);
	...
	-- -------------------------------------------------------------------------------------------------
	-- SECTION 10. UPDATE and DELETE [6 Marks]
	-- -------------------------------------------------------------------------------------------------
